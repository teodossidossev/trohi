...
React is explicitly not selected.
...
## Object-Oriented Modeling Rules

Use an object-oriented style where it improves clarity, encapsulation, validation, and maintainability.
...
## Presentational View Rule

Angular components must stay presentational.
...
Business logic belongs in the feature business/application layer, domain models, services, mappers, and infrastructure adapters.
...
Ports/adapters are not only for backend communication. They may also represent browser APIs, file export, ZIP creation, local storage, future GitHub integration, future AI provider integration, or future backend APIs.
...
Use feature-local store/component store only when it improves encapsulation, testability, state transitions, or UI simplicity.
...
Core generation, validation, output definitions, models, mappers, and consistency rules must be testable without rendering Angular UI.
...
When responding to Codex feedback:
...
If you change generated output behavior:
